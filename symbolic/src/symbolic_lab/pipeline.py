...
from symbolic_lab.features.catalog import build_symbolic_feature_row
...
def write_report(rows: list[dict[str, float | str]], output_path: Path) -> None:
    ...
    content.extend(
        [
            ...
            "## Next steps",
            "",
            "- Replace placeholder feature values with music21-derived measurements.",
            "- Add Bach chorale specific validation checks.",
            "- Extend the analysis layer with PCA, UMAP, and statistical testing.",
        ]
    )
...
def run_prototype() -> dict[str, str]:
    rows = build_rows()
    table_path = results_tables_dir() / "prototype_symbolic_features.csv"
    report_path = results_reports_dir() / "prototype_symbolic_report.md"
